...
for object in range(len(detectedObjects)):
    ...
    if object == 0: print(f"Number of objects: {len(detectedObjects)}")
    ...
    if size > 0:
        ...
        objectCutted = imgBlob[yCoor - size - 25: yCoor + size + 25, xCoor - size - 25: xCoor + 25 + size]

        print(f"Area of the object number {object}: {((xCoor + 25 + size) - (xCoor - size - 25))* ( (yCoor + size + 25) - (yCoor - size - 25) )}")
        print(f"Approximate widtht of found object {object}: {((xCoor + 25 + size) - (xCoor - size - 25))}")
        print(f"Approximate height of found object {object}: {( (yCoor + size + 25) - (yCoor - size - 25) )}")


        cv.imwrite(f"detectedObjectNumber{object}.jpg", objectCutted)
        #cv.imshow(f"detectedObjectNumber{object}", objectCutted)
        cv.waitKey(80)
