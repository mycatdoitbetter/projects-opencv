...
if __name__ == "__main__":
    ...
    preview = [int(x) for x in results]


    count = 0
    for x, y in zip(y_test, preview):
        if x == y: count += 1
    ...
    preview = np.zeros(int(max(y_training[0])) + 1)
    count = 0
    ...
    for training_set, test_set, training_label, test_label in zip(x_training, x_test, y_training, y_test):
        ...
        previed = knn_classifier(training_set, test_list, training_label, k_neigbours=7)
        preview[previed] += 1

        if previed[0] == test_label: count += 1

    accuracy = count / len(y_test)

    print("Acuracia do metodo leave one out: {0:.2f}%".format(accuracy * 100))
